...
const ctxKey = (key: any) => `ctx-${key.toLowerCase()}`;

export const getContext = (req: any, rawKey: any) => {
...
export const withContext = (allowedKeys: any, middleware: any) => {
  ...
  return (req: any, evt: any) => {
    ...
    const setContext = (rawKey: any, value: any) => {
      const key = ctxKey(rawKey);
    ...
    for (const key in data) {
      res.headers.set(key, (data as any)[key]);
      rewriteURL.searchParams.set(key, (data as any)[key]);
    }
